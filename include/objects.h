...
bool evict_pool_connection(PgPool *pool, int thread_id)        _MUSTCHECK;
bool evict_user_connection(PgCredentials *user_credentials, int thread_id)        _MUSTCHECK;
bool find_server(PgSocket *client, int thread_id)              _MUSTCHECK;
bool life_over(PgSocket *server, int thread_id);
bool release_server(PgSocket *server, int thread_id) /* _MUSTCHECK */;
bool finish_client_login(PgSocket *client, int thread_id)      _MUSTCHECK;
bool check_fast_fail(PgSocket *client, int thread_id)          _MUSTCHECK;

PgSocket *accept_client(int sock, bool is_unix) _MUSTCHECK;
void disconnect_server(PgSocket *server, bool notify, int thread_id, const char *reason, ...) _PRINTF(4, 5);
void disconnect_client(PgSocket *client, bool notify, int thread_id, const char *reason, ...) _PRINTF(4, 5);
...
bool clear_outstanding_requests_until(PgSocket *server, const char types[], int thread_id) _MUSTCHECK;
